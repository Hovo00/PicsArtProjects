...
std::shared_ptr<const Operand> MatrixOperations::matrixSubtraction(const std::vector<std::shared_ptr<const Operand> >& arguments) {
    auto m1 = std::static_pointer_cast<const Matrix>(arguments[0]);
    auto m2 = std::static_pointer_cast<const Matrix>(arguments[1]);
    assert(m1->getColCount() == m2->getColCount() && m1->getRowCount() == m2->getRowCount());
    int n = m1->getRowCount();
...
std::shared_ptr<const Operand> MatrixOperations::matrixMultiplication(const std::vector<std::shared_ptr<const Operand> >& arguments) {
    auto m1 = std::static_pointer_cast<const Matrix>(arguments[0]);
    auto m2 = std::static_pointer_cast<const Matrix>(arguments[1]);
...
std::shared_ptr<const Operand> MatrixOperations::matrixDeterminant(const std::vector<std::shared_ptr<const Operand> >& arguments) {
    ...
    if (n == 2) {
        ...
        auto subDet = std::static_pointer_cast<const Float>(matrixDeterminant(std::vector<std::shared_ptr<const Operand> >{subMatrix}));
        det = det + (pow(-1, x) * matrix->at(0, x) * subDet->getValue());
        }
   }
   return std::make_shared<Float>(det);
}
std::shared_ptr<const Matrix> MatrixOperations::removeRowAndColumn(const std::shared_ptr<const Matrix>& matrix, int removeRow, int removeCol) {
...
std::shared_ptr<const Operand> MatrixOperations::matrixInverse(const std::vector<std::shared_ptr<const Operand> >& arguments) {
    auto matrix = std::static_pointer_cast<const Matrix>(arguments[0]);
    int size =  matrix->getRowCount();
    auto inverseMatrix = std::make_shared<Matrix>(size, size);
    auto determinant = std::static_pointer_cast<const Float>(matrixDeterminant(std::vector<std::shared_ptr<const Operand> >{matrix}));
    if (size == 2) {
    ...
    for (int i = 0; i < size; ++i) {
        for (int j = 0; j < size; ++j) {
            ...
            auto coFactor = std::static_pointer_cast<const Float>(matrixDeterminant(std::vector<std::shared_ptr<const Operand> >{subMatrix}));
...
std::shared_ptr<const Operand> MatrixOperations::matrixTranspose(const std::vector<std::shared_ptr<const Operand> >& arguments) {
    auto matrix = std::static_pointer_cast<const Matrix>(arguments[0]);
...
bool MatrixOperations::equalMatrix(const std::shared_ptr<const Operand>& m1, const std::shared_ptr<const Operand>& m2) {
    auto matrix1 = std::static_pointer_cast<const Matrix>(m1);
    auto matrix2 = std::static_pointer_cast<const Matrix>(m2);
...
std::shared_ptr<const Operand> MatrixOperations::matrixEqual(const std::vector<std::shared_ptr<const Operand> >& arguments) {
    auto matrix1 = std::static_pointer_cast<const Matrix>(arguments[0]);
    auto matrix2 = std::static_pointer_cast<const Matrix>(arguments[1]);
...
std::shared_ptr<const Operand> MatrixOperations::matrixFloatDivision(const std::vector<std::shared_ptr<const Operand> >& arguments) {
    auto matrix = std::static_pointer_cast<const Matrix>(arguments[0]);
    auto flt = std::static_pointer_cast<const Float>(arguments[1]);
...
std::shared_ptr<const Operand> MatrixOperations::matrixFloatAddition(const std::vector<std::shared_ptr<const Operand> >& arguments) {
    auto matrix = std::static_pointer_cast<const Matrix>(arguments[0]);
    auto flt = std::static_pointer_cast<const Float>(arguments[1]);
    int rows = matrix->getRowCount();
    int cols = matrix->getColCount();
...
std::shared_ptr<const Operand> MatrixOperations::matrixFloatMultiplication(const std::vector<std::shared_ptr<const Operand> >& arguments) {
    auto matrix = std::static_pointer_cast<const Matrix>(arguments[0]);
    auto flt = std::static_pointer_cast<const Float>(arguments[1]);
    int rows = matrix->getRowCount();
    int cols = matrix->getColCount();
...
std::shared_ptr<const Operand> MatrixOperations::matrixFloatSubtraction(const std::vector<std::shared_ptr<const Operand> >& arguments) {
    auto matrix = std::static_pointer_cast<const Matrix>(arguments[0]);
    auto flt = std::static_pointer_cast<const Float>(arguments[1]);
    int rows = matrix->getRowCount();
    int cols = matrix->getColCount();
